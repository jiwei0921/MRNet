...
if __name__=='__main__':
    ...
    rater_i = torch.randn(1, 2, 256, 256)
    Out_six_rater = [rater_i,rater_i,rater_i,rater_i,rater_i,rater_i]
    fea = torch.randn(1, 256, 128, 128)
    out = torch.randn(1, 2, 256, 256)

    flops, params = profile(model, inputs=(Out_six_rater, fea, out))
    # This FLOPs results should be consisted of ONE-pass VGGNet & New-encoder from flag=False
    print('Flops shows: %.4f G' %(flops/1e9))
    print('Param shows: %.4f M' %(params/1e6))
